        ...
        if status == 'check':
            ...
            try:
                ...
                if row_gtg_select is not None:
                    list_users = ""
                    cs.execute(f"SELECT * FROM {utl.analyze} WHERE gtg_id={row_gtg_select['id']}")
                                        ...
                                        if user.username and not user.bot:
                                            ...
                                            if not username in participants_all:
                                                ...
                                                if user.phone and not user.phone in participants_has_phone:
                                                    ...
                                                    is_phone = 1
                                                utl.insert(cs, f"INSERT INTO {utl.analyze} (user_id,username,group_id,is_real,is_fake,is_phone,is_online,created_at) VALUES ('{user.id}','{username}','{row_gtg['group_id']}','{is_real}','{is_fake}','{is_phone}','{is_online}','{timestamp}')")
                                except:
                                    pass
                            try:
