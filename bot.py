...
print(f"run: {directory}/{filename}")
...
def user_panel(update,text=None):
    ...
    update.message.reply_html(text=text,
        reply_markup={'resize_keyboard': True,'keyboard': [
            [{'text': '➕ Create Order'}, {'text': '📋 Orders'}],
            [{'text': '➕ Add Account'}, {'text': '📋 Accounts'}],
            [{'text': '➕ Add Api'}, {'text': '📋 Apis'}],
            [{'text': '➕ Create Category'}, {'text': '📋 Categories'}],
        ...
        if row_admin['gtg_per'] or is_admin:
            if ex_data[0] == 'update':
                ...
                if row_gtg is None:
                    ...
                    count_done = cs.execute(f"SELECT id FROM {utl.reports} WHERE gtg_id={row_gtg['id']} AND status='send'")
                    count_checking = cs.execute(f"SELECT id FROM {utl.reports} WHERE gtg_id={row_gtg['id']}")
                    ...
                    query.edit_message_text(
                        text=f"{output}"+
                            ...
                            f"👤 Checking / Max: [{count_checking} / {row_gtg['max_users']}]\n\n"+
                            f"{last_bot_check}"+
                ...
                if ex_data[1] == 'accounts':
                    ...
                    if not result:
                        ...
                        for row in result:
                            ...
                            output += f"📂 Category: /category_{row['id']}\n"
...
def private_process(update: Update, context: CallbackContext) -> None:
    ...
    if is_admin:
        if ex_step[0] == 'set_cache':
                ...
                if text == '⏩ skip':
                    ...
                    if row_cats is None:
                        ...
                        if row_gtg['cats'] is not None:
                            ...
                            cats = row_cats['id']
                        cs.execute(f"UPDATE {utl.gtg} SET cats='{cats}' WHERE id={row_gtg['id']}")
                ...
                if text != "✅ end ✅":
                    try:
                        ...
                        if message.text:
                            ...
                            cs.execute(f"INSERT INTO {utl.files} (gtg_id,type_message,content,message_id,created_at,uniq_id) VALUES ('{row_gtg['id']}','message','{text}','{info_msg.message_id}','{timestamp}','{uniq_id}')")
                        elif message.photo:
                            ...
                            type_message = "photo"
                            file_id = info_msg.photo[len(info_msg.photo) - 1].file_id
                        elif message.video:
                            ...
                            type_message = "video"
                            file_id = info_msg.video.file_id
                        elif message.audio:
                            ...
                            type_message = "audio"
                            file_id = info_msg.audio.file_id
                        elif message.voice:
                            ...
                            type_message = "voice"
                            file_id = info_msg.voice.file_id
                            ...
                            file_id = info_msg.document.file_id
                        else:
                            return message.reply_html(text="⛔️ Message not supported")
                        if not message.text:
                            cs.execute(f"INSERT INTO {utl.files} (gtg_id,type_message,file_id,content,message_id,created_at,uniq_id) VALUES ('{row_gtg['id']}','{type_message}','{file_id}','{txtcap}','{info_msg.message_id}','{timestamp}','{uniq_id}')")
            ...
            if row_gtg is None:
                ...
                if not message.document:
                    ...
                    try:
                        ...
                        for value in list_members:
                            cs.execute(f"INSERT INTO {utl.analyze} (username,is_real,created_at) VALUES ('{value}','1','{timestamp}')")
                ...
                if row_gtg is None:
                    ...
                    if count < 3 and text != "✅ end ✅":
                        ...
                        cs.execute(f"SELECT COUNT(*) as count FROM {utl.analyze}")
                        max_users = cs.fetchone()['count']
                        cs.execute(f"UPDATE {utl.gtg} SET status='doing',created_at='{timestamp}',updated_at='{timestamp}' WHERE id={row_gtg['id']}")
                        cs.execute(f"UPDATE {utl.users} SET step='start' WHERE user_id='{from_id}'")
                        user_panel(update,
                            text=f"✅ Order successfully registered\n\n"+
                                f"♻️ /gtg_{row_gtg['id']}"
            ...
            cs.execute(f"INSERT INTO {utl.egroup} (user_id,link,status,created_at,updated_at,uniq_id) VALUES ({from_id},'{text}','start','{timestamp}','{timestamp}','{uniq_id}')")
            ...
            os.system(f"python tl.leave.py {from_id} group")
            info_msg.delete()
            return
        elif text == '/DeleteChats':
            info_msg = message.reply_html(text=f"Pending ...")
            os.system(f"python tl.leave.py {from_id} private")
            info_msg.delete()
            return
            ...
            if row_mbots is None:
                ...
                info_msg = message.reply_html(text="Pending ...")
                os.system(f"python tl.account-status.py {from_id} sessions {row_mbots['id']}")
                info_msg.delete()
            return
            ...
            if row_mbots is None:
                ...
                info_msg = message.reply_html(text="Pending ...")
                os.system(f"python tl.account-status.py {from_id} check {row_mbots['id']}")
                message.reply_html(text=f"Delete Account: /delete_{row_mbots['id']}")
                info_msg.delete()
            return
            ...
            if row_egroup is None:
                ...
                try:
                    ...
                    message.reply_html(
                        text=f"🔻 ID: <code>{chat_id}</code> (/exgroup_{row_egroup['chat_id'][1:]})\n"+
            ...
            if row_gtg is None:
                message.reply_html(text="❌ Not found")
            elif row_gtg['status'] == 'start':
                message.reply_html(text="❌ Order has not started yet")
            else:
                type = ex_text[2]
                list_users = ""
                ...
                try:
                    if type == 'm':
                        ...
                        cs.execute(f"SELECT * FROM {utl.analyze} WHERE gtg_id={row_gtg['id']}")
                        result_analyze = cs.fetchall()
                        if not result_analyze and not os.path.exists(path):
                            message.reply_html(text="❌ No members found")
                        else:
